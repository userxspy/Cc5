...
async def ensure_indexes():
    for name, col in COLLECTIONS.items():
        try:
            ...
            logger.info(f"✅ Indexes Synced For Collection: {name}")
...
# ─────────────────────────────────────────────────────────
# 🌐 PUBLIC SEARCH API (CROSS-COLLECTION MERGE)
# ─────────────────────────────────────────────────────────
async def get_search_results(query, max_results, offset=0, lang=None, collection_type="all", bypass_count=False):
    ...
    if collection_type == "all":
        ...
        if len(sources) > 1: actual_src = "All"
        ...
        for col, cnt in [(primary, cnt_p), (cloud, cnt_c), (archive, cnt_a)]:
            ...
            if is_text: cursor = cursor.sort([("score", {"$meta": "textScore"})])
...
async def get_actor_search_results(actor_name, tags_list, max_results, offset=0, collection_type="all"):
    ...
    if tags_list and isinstance(tags_list, list):
        for t in tags_list:
            if t and str(t).strip(): all_terms.append(str(t).strip())
    ...
    except Exception: regex = re.compile(re.escape(actor_name) if actor_name else "NO_ACTOR_MATCH", flags=re.IGNORECASE)
